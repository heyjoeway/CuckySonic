...
static const SDL_Scancode keyToScancode[IBK_MAX] = {
...
static const SDL_GameControllerButton buttonToSDLButton[IBB_MAX] = {
...
SDL_GameController *controller[CONTROLLERS];

bool Backend_IsKeyDown(INPUTBINDKEY key)
{
	return keyboardState[keyToScancode[key]];
}

bool Backend_IsButtonDown(size_t index, INPUTBINDBUTTON button)
{
	SDL_GameController *chkController = controller[index];
	if (chkController != nullptr)
		return SDL_GameControllerGetButton(chkController, buttonToSDLButton[button]);
	return false;
}
...
void Backend_GetAnalogueStick(size_t index, int16_t *x, int16_t *y)
{
	...
	if (chkController != nullptr)
	{
		*x = SDL_GameControllerGetAxis(chkController, SDL_CONTROLLER_AXIS_LEFTX);
		*y = SDL_GameControllerGetAxis(chkController, SDL_CONTROLLER_AXIS_LEFTY);
	}
}
	...
	while (SDL_PollEvent(nullptr) || (focusYield && (!exit)))
	{
		...
		switch (event.type)
		{
			...
			case SDL_CONTROLLERDEVICEREMOVED: //Controller (not joystick) disconnecting
				SDL_GameController *discController = SDL_GameControllerFromInstanceID(event.cdevice.which);
				for (size_t i = 0; i < CONTROLLERS; i++)
					if (controller[i] == discController)
						controller[i] = nullptr;
				SDL_GameControllerClose(discController);
				break;
		}
	}
	
	//Update backend input state
	Backend_UpdateInputState();
	return exit;
}
